...
const AdminBotUserTrades = () => {
  ...
  const fetchUserTradesForBot = async (botId) => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/api/bots/${botId}/user-trades`, {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
        },
  ...
  return (
    <Container>
      ...
      {loading ? (
        ...
        bots.map((bot) => (
          <Accordion key={bot.id} expanded={expandedBot === bot.id} onChange={() => handleExpandBot(bot.id)}>
              ...
              {botTrades[bot.id] ? (
                botTrades[bot.id].map((userData) => (
                  <div key={userData.user.id}>
                    <Typography variant="h6">{userData.user.name}'s Trades</Typography>
                    {userData.trades.length > 0 ? (
                          ...
                          <TableBody>
                            {userData.trades.map((trade) => (
                              <TableRow key={trade.id}>
                                ...
                                <TableCell>{trade.buy_at ? new Date(trade.buy_at).toLocaleString() : "N/A"}</TableCell>
                                <TableCell>{trade.sold_at ? new Date(trade.sold_at).toLocaleString() : "N/A"}</TableCell>
                              </TableRow>
                            ))}
                          </TableBody>
                        </Table>
                      ...
                      <Typography>No trades available for this user.</Typography>
                    )}
                  </div>
                ))
              ) : (
